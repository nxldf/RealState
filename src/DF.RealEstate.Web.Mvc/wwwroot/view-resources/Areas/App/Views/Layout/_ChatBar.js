...
var chat = {
  ...
  loadLastState: function () {
    app.localStorage.getItem('app.chat.isOpen', function (isOpen) {
      ...
      if (chat.isOpen) {
        $('body, #kt_quick_sidebar')
          .addClass('offcanvas-on')
          .promise()
          .done(function () {
  ...
  initConversationScrollbar: function () {
    ...
    KTUtil.scrollInit(scrollEl, {
      ...
      height: function () {
        ...
        if (KTUtil.isBreakpointDown('lg')) {
          ...
          if (scrollEl) {
            height =
              height - parseInt(KTUtil.css(scrollEl, 'margin-top')) - parseInt(KTUtil.css(scrollEl, 'margin-bottom'));
          }
    ...
    var height =
      $('#kt_quick_sidebar').outerHeight(true) -
      $('#kt_chat_content .card-header').outerHeight(true) -
      $('#kt_chat_content .card-footer').outerHeight(true) -
      50;
  ...
  bindUiEvents: function () {
    ...
    $('div#kt_quick_sidebar').on('mouseleave', function (e) {
      if (chat.pinned || $(e.target).attr('data-toggle') === 'popover') {
