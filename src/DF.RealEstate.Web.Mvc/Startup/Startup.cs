...
using DF.RealEstate.Configure;
using DF.RealEstate.EntityFrameworkCore;
using DF.RealEstate.Identity;
using DF.RealEstate.Schemas;
using DF.RealEstate.Web.Chat.SignalR;
using DF.RealEstate.Web.Common;
using DF.RealEstate.Web.Resources;
using Swashbuckle.AspNetCore.Swagger;
using DF.RealEstate.Web.IdentityServer;
using DF.RealEstate.Web.Swagger;
using Stripe;
...
using DF.RealEstate.Web.HealthCheck;
using Owl.reCAPTCHA;
using HealthChecksUISettings = HealthChecks.UI.Configuration.Settings;
using Microsoft.AspNetCore.Server.Kestrel.Https;
using Microsoft.Extensions.DependencyInjection.Extensions;
using DF.RealEstate.Web.Extensions;
using SecurityStampValidatorCallback = DF.RealEstate.Identity.SecurityStampValidatorCallback;

namespace DF.RealEstate.Web.Startup
{
    public class Startup
    {
        ...
        public IServiceProvider ConfigureServices(IServiceCollection services)
        {
            ...
            return services.AddAbp<RealEstateWebMvcModule>(options =>
            {
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, ILoggerFactory loggerFactory)
        {
            ...
            if (env.IsDevelopment())
            {
                ...
                app.UseRealEstateForwardedHeaders();
            }
            else
            {
            ...
            if (WebConsts.SwaggerUiEnabled)
            {
                ...
                {
                    options.SwaggerEndpoint(_appConfiguration["App:SwaggerEndPoint"], "RealEstate API V1");
                    options.IndexStream = () => Assembly.GetExecutingAssembly()
                        .GetManifestResourceStream("DF.RealEstate.Web.wwwroot.swagger.ui.index.html");
        ...
        private void ConfigureSwagger(IServiceCollection services)
        {
            services.AddSwaggerGen(options =>
            {
                ...
                var applicationXml = $"DF.RealEstate.Application.xml";
                var applicationXmlPath = Path.Combine(AppContext.BaseDirectory, applicationXml);
                options.IncludeXmlComments(applicationXmlPath);

                var webCoreXmlFile = $"DF.RealEstate.Web.Core.xml";
