...
namespace DF.RealEstate.Web.Public.Startup
{
    [DependsOn(
        typeof(RealEstateWebCoreModule)
    )]
    public class RealEstateWebFrontEndModule : AbpModule
    {
        private readonly IConfigurationRoot _appConfiguration;

        public RealEstateWebFrontEndModule(IWebHostEnvironment env, RealEstateEntityFrameworkCoreModule abpZeroTemplateEntityFrameworkCoreModule)
        {
        ...
        {
            IocManager.RegisterAssemblyByConvention(typeof(RealEstateWebFrontEndModule).GetAssembly());
        }
    }
}
